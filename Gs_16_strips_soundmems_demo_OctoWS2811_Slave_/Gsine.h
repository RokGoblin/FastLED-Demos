...
// Local variables. Play around with these.
  uint8_t allfreq = 33;                                                         // G! originally 32 //You can change the frequency, thus distance between bars. Wouldn't recommend changing on the fly.
...
//  thisphase += sampleavg/2;                                                     // Move the sine waves along as a function of sound.
  ...
  thisphase = beatsin16(300,-600, 600);                                            //G! originally 20, -600, 600

  
  colorIndex = millis() >> 6;                                                   // G! Originally >> 4 // millis() can be used for so many things.
