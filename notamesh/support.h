...
void addGlitter(fract8 chanceOfGlitter) {                                      // Let's add some glitter
  ...
  if( random8() < chanceOfGlitter) {
    leds[random16(NUM_LEDS)] += (64,64,64);
  }
...
// Mydata = ((mydata & 0x80)?0x01:0x00) | (mydata << 1);    // ROL or 0x8000 for 16 bit for single bit only.
// Mydata = ((mydata & 0x01)?0x80:0x00) | (mydata >> 1);    // ROR or 0x8000 for 16 bit for single bit only.
