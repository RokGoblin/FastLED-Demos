...
DMAMEM int displayMemory[NUM_LEDS*6];  // G! added sync stuff from WS2811_VideoDisplay example
int drawingMemory[NUM_LEDS*6];  // G! added sync stuff from WS2811_VideoDisplay example
elapsedMicros elapsedUsecSinceLastFrameSync = 0;  // G! added sync stuff from WS2811_VideoDisplay example
...
/*LEDS.addLeds<LED_TYPE, LED_DT9, COLOR_ORDER>(leds, NUM_LEDS).setCorrection(TypicalLEDStrip);          // Use this for WS2812
 ...
 LEDS.addLeds<LED_TYPE, LED_DT16, COLOR_ORDER>(leds, NUM_LEDS).setCorrection(TypicalLEDStrip);          // Use this for WS2812
  ...
  Serial.begin(1000000);                                                          // Serial port speed for debugging.
{  // G! added sync stuff from WS2811_VideoDisplay example
  pinMode(12, INPUT_PULLUP); // Frame Sync  // G! added sync stuff from WS2811_VideoDisplay example
  Serial.setTimeout(50);  // G! added sync stuff from WS2811_VideoDisplay example
}
} // setup()
...
#include "VideoDisplay.h"  // G! added new header file

void loop() {                                                                   // The >>>>>>>>>> L-O-O-P <<<<<<<<<<<<<<<<<<<<<<<<<<<<  is buried way down here!!!11!1!
  VideoDisplay();  // G! adding for sync, moved code to VideoDisplay.h file
  soundmems();
...
//  leds.begin();  // G! added sync stuff from WS2811_VideoDisplay example
//  leds.show();  // G! added sync stuff from WS2811_VideoDisplay example
