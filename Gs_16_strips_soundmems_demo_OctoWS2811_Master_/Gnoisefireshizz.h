#ifndef GNOISEFIRESHIZZ_H
#define GNOISEFIRESHIZZ_H

void Gnoisefireshizz() {                                                          // Create fire based on noise and sampleavg. 
...
//    leds[NUM_LEDS-i+1] = ColorFromPalette(currentPalette, index, sampleavg, NOBLEND);      // G! Stuff  // With that value, look up the 8 bit colour palette value and assign it to the current LED.                                                                                 
//    leds[NUM_LEDS+i-1] = ColorFromPalette(currentPalette, index, sampleavg, NOBLEND);      // G! Stuff // With that value, look up the 8 bit colour palette value and assign it to the current LED.                                                                                 
